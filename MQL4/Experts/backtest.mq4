...
extern string PROJECT_NAME = "htr";
         ...
         if(ticket>0)
           {
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) {
               InformPullClient(pSocket, StringFormat("ticket | %d", compArray[8]));
...
void OnTick()
  {
   processing = 1;
   ZmqMsg tick(GetBidAsk(Symbol()));
     
   //pushSocket.send(tick,true); // NON-BLOCKING
   pushSocket.send(tick,false); // BLOCKING
   while(processing == 1) {}     
  }
//+------------------------------------------------------------------+
