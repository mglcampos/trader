...
class EndpointsMixin(object):
    ...
    def create_account(self, **params):
        """ Create an account. Valid only in histdata.
...
class API(EndpointsMixin, object):
    def __init__(self, environment="practice", access_token=None, headers=None):
        ...
        :param environment: (optional) Provide the environment for oanda's REST api, either 'histdata', 'practice', or 'live'. Default: practice
        :param access_token: (optional) Provide a valid access token if you have one. This is required if the environment is not histdata.
        """

        if environment == 'histdata':
            self.api_url = 'http://api-histdata.oanda.com'
...
class OandaFormat(object):

    def formatInstruments(self, instruments):
        string = ''
        for i in range(len(instruments)):
            if string == '':
                string = instruments[i].replace('/', '_', 1)
            else:
                string = string + ',' + instruments[i].replace('/', '_', 1)
        print(string)
        return string
